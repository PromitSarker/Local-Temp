...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Create Transfer (Payout)
    const transfer = await stripe.transfers.create({
        amount: transferAmount,
        currency: 'gbp',
        destination: locumAccountId,
        transfer_group: bookingId, // Link transfer to booking
        metadata: {
    ...
    await supabaseClient
        ...
        .update({ 
            ...
            admin_fee: adminFee / 100, // Store as decimal (e.g. 10.50)
            status: 'completed' // Mark booking as completed when funds are released
        })
        .eq('id', bookingId)
